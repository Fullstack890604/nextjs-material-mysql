...
import { isValidEmail, isValidVNPhone, sanitizePhoneInput } from "@/lib/validators";
...
export default function DepartmentsPage() {
  ...
  const search = usePageSearch("Tìm theo mã, tên, trưởng phòng, địa điểm...");
  ...
  // Danh mục phụ cho ô chọn trong form (địa điểm, trưởng phòng) — tải một lần
  const loadLookups = useCallback(async () => {
    try {
      ...
      const [locData, staffData] = await Promise.all([locRes.json(), staffRes.json()]);
      setLocations(locData.status === "OK" ? locData.data || [] : []);
  ...
  // SĐT: chỉ cho gõ chữ số (và dấu "+" ở đầu), tự chặn độ dài
  const handlePhoneChange = (e) =>
    setForm((f) => ({ ...f, phone: sanitizePhoneInput(e.target.value) }));

  const phoneError = !!form.phone.trim() && !isValidVNPhone(form.phone);
  const emailError = !!form.email.trim() && !isValidEmail(form.email);

  const handleSave = async () => {
    if (!form.code.trim()) return notify("Vui lòng nhập mã phòng ban", "warning");
    if (!form.name.trim()) return notify("Vui lòng nhập tên phòng ban", "warning");
    if (phoneError) return notify("Số điện thoại không hợp lệ", "warning");
    if (emailError) return notify("Email không hợp lệ", "warning");
    setSaving(true);
    try {
      ...
      const url = editing
        ? `/api/departments/${encodeURIComponent(editing)}`
        : "/api/departments";
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/departments/${encodeURIComponent(deleteTarget.code)}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.status === "OK") {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <PageHeader
        ...
        subtitle="Quản lý danh sách phòng ban và định biên nhân sự"
        actions={
          <>
          ...
          <Table size="small" sx={{ minWidth: 1080 }}>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={headCellSx}>Tên phòng ban</TableCell>
                <TableCell sx={headCellSx}>Trưởng phòng</TableCell>
                <TableCell sx={headCellSx}>Địa điểm</TableCell>
                <TableCell sx={headCellSx}>Liên hệ</TableCell>
            ...
            <TableBody>
              ...
              {!loading && filtered.length > 0 && (
                <TableRow>
                  <TableCell sx={totalCellSx} colSpan={6}>
                    TỔNG CỘNG{" "}
                    <Typography variant="caption" color="text.secondary">
                      ({qty(totals.count)} phòng ban, {qty(totals.active)} đang hoạt động)
                    </Typography>
                  </TableCell>
      ...
      <Dialog open={dialogOpen} onClose={(e, reason) => reason !== "backdropClick" && setDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        >
          {editing ? <Edit /> : <Add />}
          {editing ? "Cập nhật" : "Thêm mới"} phòng ban
        </DialogTitle>
      ...
      <ConfirmDialog
        ...
        message={
          <Typography variant="body2">
            Bạn có chắc muốn xóa phòng ban <strong>{deleteTarget?.name}</strong>? Hành động này không thể hoàn tác.
          </Typography>
        }
      />
