...
export default function PositionsPage() {
  ...
  const handleSave = async () => {
    if (!form.code.trim()) return notify("Vui lòng nhập mã chức vụ", "warning");
    if (!form.name.trim()) return notify("Vui lòng nhập tên chức vụ", "warning");
    setSaving(true);
    try {
      ...
      const url = editing ? `/api/positions/${encodeURIComponent(editing)}` : "/api/positions";
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/positions/${encodeURIComponent(deleteTarget.code)}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.status === "OK") {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <PageHeader
        ...
        subtitle="Quản lý danh sách chức vụ nhân sự"
        // icon={<Work />}
        actions={
          <>
          ...
          <Table size="small" sx={{ minWidth: 680 }}>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={headCellSx}>Tên chức vụ</TableCell>
      ...
      <Dialog open={dialogOpen} onClose={(e, reason) => reason !== "backdropClick" && setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        >
          {editing ? <Edit /> : <Add />}
          {editing ? "Cập nhật" : "Thêm mới"} chức vụ
        </DialogTitle>
      ...
      <ConfirmDialog
        ...
        message={
          <Typography variant="body2">
            Bạn có chắc muốn xóa chức vụ <strong>{deleteTarget?.name}</strong>? Hành động này không thể hoàn tác.
          </Typography>
        }
      />
