...
export default function CompaniesPage() {
  ...
  const search = usePageSearch("Tìm theo mã, tên, MST, người đại diện...");
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        ...
        r.representative?.toLowerCase().includes(q) ||
  ...
  const handleSave = async () => {
    if (!form.code.trim()) return notify("Vui lòng nhập mã công ty", "warning");
    if (!form.name.trim()) return notify("Vui lòng nhập tên công ty", "warning");
    setSaving(true);
    try {
      const payload = {
        ...form,
      ...
      const url = editing ? `/api/companies/${encodeURIComponent(editing)}` : "/api/companies";
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/companies/${encodeURIComponent(deleteTarget.code)}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.status === "OK") {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <PageHeader
        ...
        subtitle="Quản lý thông tin các công ty"
        // icon={<Business />}
        actions={
          <>
            <Button
          ...
          <Table size="small" sx={{ minWidth: 900 }}>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={headCellSx}>Tên công ty</TableCell>
                <TableCell sx={headCellSx}>Người đại diện</TableCell>
                <TableCell sx={headCellSx}>MST</TableCell>
              ...
              {!loading &&
                paged.map((r, index) => (
                  <TableRow key={r.code} hover>
                    ...
                    <TableCell>{r.representative || "—"}</TableCell>
      ...
      <Dialog open={dialogOpen} onClose={(e, reason) => reason !== "backdropClick" && setDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        >
          {editing ? <Edit /> : <Add />}
          {editing ? "Cập nhật" : "Thêm mới"} công ty
        </DialogTitle>
        <DialogCloseButton onClick={() => setDialogOpen(false)} />
        <DialogContent dividers>
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Mã công ty" value={form.code} onChange={setField("code")} fullWidth required disabled={!!editing}
                slotProps={{ input: { startAdornment: (<InputAdornment position="start"><Tag fontSize="small" /></InputAdornment>) } }} />
            </Grid>
            <Grid size={{ xs: 12, sm: 8 }}>
              <TextField label="Tên công ty" value={form.name} onChange={setField("name")} fullWidth required
                slotProps={{ input: { startAdornment: (<InputAdornment position="start"><Business fontSize="small" /></InputAdornment>) } }} />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Tên viết tắt" value={form.shortName} onChange={setField("shortName")} fullWidth
                slotProps={{ input: { startAdornment: (<InputAdornment position="start"><BadgeIcon fontSize="small" /></InputAdornment>) } }} />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Mã số thuế" value={form.taxCode} onChange={setField("taxCode")} fullWidth
                slotProps={{ input: { startAdornment: (<InputAdornment position="start"><ReceiptLong fontSize="small" /></InputAdornment>) } }} />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Người đại diện" value={form.representative} onChange={setField("representative")} fullWidth
                slotProps={{ input: { startAdornment: (<InputAdornment position="start"><Person fontSize="small" /></InputAdornment>) } }} />
            </Grid>
            <Grid size={{ xs: 12 }}>
              <TextField label="Địa chỉ" value={form.address} onChange={setField("address")} fullWidth
                slotProps={{ input: { startAdornment: (<InputAdornment position="start"><Place fontSize="small" /></InputAdornment>) } }} />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Điện thoại" value={form.phone} onChange={setField("phone")} fullWidth
            ...
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Website" value={form.website} onChange={setField("website")} fullWidth
                slotProps={{ input: { startAdornment: (<InputAdornment position="start"><Language fontSize="small" /></InputAdornment>) } }} />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
            ...
            <Grid size={{ xs: 12, sm: 8 }}>
              <Stack direction="row" alignItems="center" sx={{ height: "100%" }}>
                <FormControlLabel control={<Switch checked={form.isActive} onChange={setSwitch("isActive")} />} label="Đang hoạt động" />
              </Stack>
            </Grid>
            <Grid size={{ xs: 12 }}>
              <TextField label="Ghi chú" value={form.note} onChange={setField("note")} fullWidth multiline minRows={2}
      ...
      <ConfirmDialog
        ...
        message={
          <Typography variant="body2">
            Bạn có chắc muốn xóa công ty <strong>{deleteTarget?.name}</strong>? Hành động này không thể hoàn tác.
          </Typography>
        }
      />
